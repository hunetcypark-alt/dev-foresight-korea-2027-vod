...
const STATES = {
  ...
  vodsale: {
    ...
    hero: {
      cta: 'VOD 구매하기',
      variant: 'vod',
      hint: { dim:'', main:'<span style="font-family:Pretendard,sans-serif;font-size:14px;font-weight:400;color:rgba(255,255,255,.7);line-height:1.6;letter-spacing:-.5px;text-align:center;display:block">포럼 참석자분들은 상단 <strong>[VOD] &gt; [다시보기]</strong> 메뉴를 통해<br>구매없이 시청이 가능합니다.</span>' },
      sub: 'AX · Transforming into an AI-Native Company',
    },
...
function applyState(key){
  ...
  // GNB items
  const map = { events:'gnb-events', register:'gnb-register', webinar:'gnb-webinar' };
  ...
  // 모바일 웨비나 링크 동기화
  const mobWb = document.getElementById('mob-nav-webinar');
  if(mobWb) mobWb.classList.toggle('is-hidden', !s.gnb.webinar);
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  let init = 'vodsale';
  try{ localStorage.removeItem('fk27_state'); }catch(_){}
...
(function(){
  function switchPage(id){
    ...
    /* Register 페이지 → 항상 등록하기 탭으로 초기화 */
    if(id === 'register') switchRegTab('register');
  }
...
/* ── Sidebar 액티브 탭 자동 스크롤 (tablet · mobile) ── */
(function(){
  var sidebar = document.getElementById('pg-sidebar');
  if(!sidebar) return;
  var mo = new MutationObserver(function(){
    if(window.innerWidth >= 1024) return;
    var active = sidebar.querySelector('.pg-sb-item.active');
    if(!active) return;
    var nav = sidebar.querySelector('.pg-sb-nav');
    if(!nav) return;
    var navW = nav.offsetWidth;
    var itemL = active.offsetLeft;
    var itemW = active.offsetWidth;
    nav.scrollTo({left: itemL - (navW - itemW) / 2, behavior:'smooth'});
  });
  mo.observe(sidebar, {attributes:true, subtree:true, attributeFilter:['class']});
})();
...
/* ── 웨비나 페이지 ── */
(function(){
  var authEl   = document.getElementById('wb-auth');
  var streamEl = document.getElementById('wb-stream');
  var submitBtn = document.getElementById('wb-auth-submit');
  var tabsEl   = document.getElementById('wb-tabs');

  var SP  = 'assets/img/speakers/';
  var MAN = 'assets/img/man.svg';

  var TRACK_INFO = {
    keynote: { label:'KEYNOTE', listTrack:'KEYNOTE', listTitle:'기조연설',
      sessions:[
        { time:'09:00~09:50', title:'AX: Transforming into an AI-Native Company', org:'LG AI연구원', name:'이홍락 CSAI', img:'assets/img/ceo-photo.png', live:true }
      ]},
    a: { label:'TRACK A', listTrack:'TRACK A', listTitle:'거시환경 — 2027 전략 수립의 외부 좌표',
      sessions:[
        { time:'10:10~11:00', title:'2027 사업계획의 첫 번째 전제: 성장·물가·교역의 새 질서',   org:'대외경제정책연구원', name:'윤상하 실장',                img:SP+'img.png',    live:true },
        { time:'11:20~12:10', title:'모두가 타깃이 되는 세계: 2027 지정학 리스크와 한국 기업의 선택', org:'중앙대학교',        name:'이승주 교수',                img:SP+'img-1.png'  },
        { time:'13:30~14:20', title:'에너지·물류 초크포인트: 원가와 공급망을 흔드는 다음 충격',    org:'삼일PwC',          name:'강명수 국제통상솔루션센터장', img:SP+'img-2.png'  },
        { time:'14:40~15:30', title:'환율·금리·자본비용: 2027 재무 전략의 리스크 지도',           org:'',                 name:'TBD',                        img:MAN, tbd:true   },
        { time:'15:50~16:40', title:'K자형 양극화의 한국 경제: 2027 성장 기회와 방어 전략',       org:'현대경제연구원',    name:'주원 실장',                  img:SP+'img-4.png'  },
        { time:'17:00~17:50', title:'불확실성을 숫자로 바꾸는 시나리오 경영: 예산과 실행계획의 기술', org:'',               name:'TBD',                        img:MAN, tbd:true   }
      ]},
    b: { label:'TRACK B', listTrack:'TRACK B', listTitle:'경영전략·혁신 — AI Native Company의 전략 공식',
      sessions:[
        { time:'10:10~11:00', title:'연간 계획의 종말: AI Adaptive Planning으로 전략을 다시 짜다',            org:'', name:'TBD', img:MAN, tbd:true },
        { time:'11:20~12:10', title:'AI Native 도약의 첫 조건: 성장을 막는 레거시를 버리는 법',              org:'', name:'TBD', img:MAN, tbd:true },
        { time:'13:30~14:20', title:'Dual AX 전략: 사업 모델과 실행 역량을 동시에 바꾸다',                   org:'', name:'TBD', img:MAN, tbd:true },
        { time:'14:40~15:30', title:'[AX사례] 제품을 넘어 생태계로: 플랫폼 기업은 어떻게 비즈니스를 재정의했나', org:'', name:'TBD', img:MAN, tbd:true },
        { time:'15:50~16:40', title:'[AX사례] 실행 속도의 혁명: 하이퍼 스냅스 조직은 어떻게 움직이는가',      org:'', name:'TBD', img:MAN, tbd:true },
        { time:'17:00~17:50', title:"실리콘밸리의 극단적 AX: '1인 유니콘'이 던지는 성장의 질문",              org:'', name:'TBD', img:MAN, tbd:true }
      ]},
    c: { label:'TRACK C', listTrack:'TRACK C', listTitle:'AX·Tech — AI Agent, 데이터, 플랫폼',
      sessions:[
        { time:'10:10~11:00', title:'스틸칼라의 등장: 피지컬 AI가 산업 현장을 바꾸는 방식',              org:'',        name:'TBD',           img:MAN, tbd:true  },
        { time:'11:20~12:10', title:'테크 드래곤의 비상: 중국 AI 굴기가 바꾸는 경쟁의 판',               org:'한양대학교', name:'백서인 교수',   img:SP+'img-26.png' },
        { time:'13:30~14:20', title:'AI 투자의 성패: 거품을 넘어 ROI를 만드는 법',                      org:'',        name:'TBD',           img:MAN, tbd:true  },
        { time:'14:40~15:30', title:'[AX사례] 전사 AI 도입의 현실: 성공 조건과 실패 패턴',              org:'KPMG',    name:'이동근 AI센터장', img:SP+'img-28.png' },
        { time:'15:50~16:40', title:'자율 경제 에이전트(AEA): AI가 결정하는 시대의 통제와 책임',          org:'',        name:'TBD',           img:MAN, tbd:true  },
        { time:'17:00~17:50', title:'통제인가 자율인가: 새도우 AI를 성과로 바꾸는 조직 설계',             org:'',        name:'TBD',           img:MAN, tbd:true  }
      ]},
    d: { label:'TRACK D', listTrack:'TRACK D', listTitle:'마케팅 — AI가 고객 접점을 재정의한다',
      sessions:[
        { time:'10:10~11:00', title:'AI 네이티브 소비자의 탄생: 2027 고객은 어떻게 선택하는가',            org:'트렌드코리아컴퍼니', name:'전미영 박사',   img:MAN               },
        { time:'11:20~12:10', title:'검색 이후의 브랜드 전략: AI가 바꾼 구매 여정의 새 규칙',             org:'Kearney',        name:'윤성훈 파트너', img:SP+'img-12.png'   },
        { time:'13:30~14:20', title:'AI 시대의 미디어 투자 전략: 채널 믹스와 성과 극대화의 새 공식',       org:'한국종합예술원',   name:'이승무 교수',   img:MAN               },
        { time:'14:40~15:30', title:'[AX사례] 퍼포먼스 마케팅의 재설계: AI 측정·최적화와 인간의 판단',    org:'고려대학교',      name:'송수진 교수',   img:MAN               },
        { time:'15:50~16:40', title:'AI 네이티브 마케팅 조직: 사람과 에이전트는 어떻게 협업하는가',        org:'',               name:'TBD',           img:MAN, tbd:true     },
        { time:'17:00~17:50', title:'[AX사례] 고객 경험의 AX: AI는 접점을 어떻게 바꾸는가',             org:'',               name:'TBD',           img:MAN, tbd:true     }
      ]},
    e: { label:'TRACK E', listTrack:'TRACK E', listTitle:'조직·리더십 — AI Native 조직을 만드는 법',
      sessions:[
        { time:'10:10~11:00', title:'글로벌 빅테크가 다시 짠 일의 구조: AI Native 직무 재설계',          org:'동국대학교', name:'이중학 교수',   img:SP+'img-22.png', live:true },
        { time:'11:20~12:10', title:'프롬프트가 업무 능력이 된 시대: 임직원 AI 리스킬링 전략',             org:'',          name:'TBD',           img:MAN, tbd:true             },
        { time:'13:30~14:20', title:'프롬프트 리더십: 구성원의 AI 잠재력을 깨우는 리더의 역할',            org:'',          name:'TBD',           img:MAN, tbd:true             },
        { time:'14:40~15:30', title:'[AX사례] AI Native HR: 채용부터 평가까지 인재 의사결정을 바꾸다',   org:'SAP',       name:'한광모 본부장',  img:SP+'img-21.png'           },
        { time:'15:50~16:40', title:'[AX사례] 전사 AI 내재화의 조건: 임원진은 어떻게 변화를 밀어붙였나',  org:'',          name:'TBD',           img:MAN, tbd:true             },
        { time:'17:00~17:50', title:'[AX사례] 연공서열과 AI 속도의 충돌: 한국형 조직의 현실적 전환법',    org:'',          name:'TBD',           img:MAN, tbd:true             }
      ]}
  };

  function renderSessions(track){
    var info = TRACK_INFO[track];
    var ul   = document.getElementById('wb-sessions');
    if(!info || !ul) return;
    var now = info.sessions[0];
    ul.innerHTML = info.sessions.map(function(s){
      var isTbd  = !!s.tbd;
      var isLive = !!s.live;
      var cls    = 'wb-sess ' + (isLive ? 'wb-sess--live' : 'wb-sess--upcoming');
      var imgCls = isTbd ? ' class="wb-sess-tbd"' : '';
      var badge  = isLive ? '<span class="wb-live-badge"><span class="wb-live-dot"></span>LIVE</span>' : '';
      var spk    = isTbd
        ? '<span>섭외 진행 중</span><strong>TBD</strong>'
        : '<span>' + (s.org || '') + '</span><strong>' + s.name + '</strong>';
      return '<li class="' + cls + '">'
        + '<div class="wb-sess-thumb"><img src="' + s.img + '" alt="' + s.name + '"' + imgCls + '>' + badge + '</div>'
        + '<div class="wb-sess-info">'
        + '<p class="wb-sess-time">' + s.time + '</p>'
        + '<h4 class="wb-sess-title">' + s.title + '</h4>'
        + '<div class="wb-sess-speaker">' + spk + '</div>'
        + '</div></li>';
    }).join('');
    var nowTrack  = document.getElementById('wb-now-track');
    var nowTitle  = document.getElementById('wb-now-title');
    var nowOrg    = document.getElementById('wb-now-org');
    var nowName   = document.getElementById('wb-now-name');
    var dlLabel   = document.getElementById('wb-dl-label');
    var listTrack = document.getElementById('wb-list-track');
    var listTitle = document.getElementById('wb-list-title');
    if(nowTrack)  nowTrack.textContent  = info.label;
    if(nowTitle)  nowTitle.textContent  = now.title;
    if(nowOrg)    nowOrg.textContent    = now.org || '';
    if(nowName)   nowName.textContent   = now.name;
    if(dlLabel)   dlLabel.textContent   = info.label + ' 교안 받기';
    if(listTrack) listTrack.textContent = info.listTrack;
    if(listTitle) listTitle.textContent = info.listTitle;
  }

  function switchWbTrack(track){
    if(!TRACK_INFO[track]) return;
    if(tabsEl){
      tabsEl.querySelectorAll('.wb-tab').forEach(function(t){
        var isOn = t.dataset.track === track;
  ...
  if(tabsEl){
    tabsEl.addEventListener('click', function(e){
      var btn = e.target.closest('.wb-tab');
      if(!btn) return;
      switchWbTrack(btn.dataset.track);
    });
  }
  ...
  if(submitBtn){
    submitBtn.addEventListener('click', function(){
      var name  = (document.getElementById('wb-name')  || {}).value || '';
      var phone = (document.getElementById('wb-phone') || {}).value || '';
      if(!name.trim()){ alert('성함을 입력해 주세요.'); return; }
      if(!phone.trim()){ alert('휴대폰 번호를 입력해 주세요.'); return; }
      if(authEl)   authEl.hidden   = true;
      if(streamEl) streamEl.hidden = false;
      renderSessions('a');
      try{ sessionStorage.setItem('wb_authed','1'); }catch(_){}
    });
  }

  /* switchPage 시 인증 상태 복원 */
  var _origSwitch = window.switchPage;
  if(_origSwitch){
    window.switchPage = function(id){
      _origSwitch(id);
      if(id === 'webinar' && authEl && streamEl){
        var authed = false;
        try{ authed = !!sessionStorage.getItem('wb_authed'); }catch(_){}
        authEl.hidden   = authed;
        streamEl.hidden = !authed;
        if(authed) renderSessions('a');
      }
    };
  }
})();
...
(function(){
  document.addEventListener('DOMContentLoaded', function() {
    var vodPage = document.getElementById('page-vod');
    if (!vodPage) return;

    var totalEl = vodPage.querySelector('.vod-total dd');

    function getTracks() {
      return vodPage.querySelectorAll('.vod-track');
    }
    ...
    function updateTotal() {
      ...
      getTracks().forEach(function(t) {
        if (t.classList.contains('selected')) {
          sum += parseInt(t.dataset.price, 10) || 0;
        }
      });
      if (totalEl) totalEl.textContent = sum.toLocaleString('ko-KR') + '원';
    }

    function setSelected(track, on) {
      var check = track.querySelector('.vod-check');
      track.classList.toggle('selected', on);
      if (check) check.classList.toggle('checked', on);
    }

    vodPage.addEventListener('click', function(e) {
      var track = e.target.closest('.vod-track');
      if (!track) return;

      var isAll = track.dataset.track === 'all';
      var willSelect = !track.classList.contains('selected');
      var tracks = getTracks();

      if (isAll) {
        tracks.forEach(function(t) { setSelected(t, t === track ? willSelect : false); });
      } else {
        tracks.forEach(function(t) { if (t.dataset.track === 'all') setSelected(t, false); });
        setSelected(track, willSelect);
      }
      updateTotal();
    });
...
// 연사 카드 다시보기 버튼 주입
document.addEventListener('DOMContentLoaded', function() {
  var playIcon = '<svg width="12" height="12" viewBox="0 0 16 16" fill="none"><path d="M6 4.5L11 8 6 11.5V4.5Z" fill="currentColor"/><circle cx="8" cy="8" r="6.5" stroke="currentColor" stroke-width="1.5"/></svg>';
  document.querySelectorAll('#page-program .sc').forEach(function(sc) {
    if (sc.querySelector('.sc-vod-btn')) return;
    var btn = document.createElement('a');
    btn.href = '#';
    btn.className = 'sc-vod-btn';
    btn.innerHTML = playIcon + '다시보기';
    sc.appendChild(btn);
  });
});
